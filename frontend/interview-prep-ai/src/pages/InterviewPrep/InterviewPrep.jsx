...
const RoleInfoHeader = lazy(() => import('./components/RoleInfoHeader'));
const QuestionCard = lazy(() => import('../../components/Cards/QuestionCard'));
const AIResponsePreview = lazy(() => import('./components/AIResponsePreview'));
const SpinnerLoader = lazy(() => import('../../components/Loader/SpinnerLoader'));
const SkeletonLoader = lazy(() => import('../../components/Loader/SkeletonLoader'));
...
const InterviewPrep = ({ sessionId }) => {
  ...
  const generateConceptExplanation = async (question, index) => {
    setErrorMsg(""); setExplanation(null); setActiveQuestionIndex(index); setIsLoading(true); setShowExplanationModal(true);
    try {
      ...
      setExplanation(null); setErrorMsg("Failed to generate explanation. Try again later."); console.log(error);
  ...
  const toggleQuestionPinStatus = async (questionId) => {
    try {
      ...
      console.log(error); toast.error("Failed to update question status");
    }
  };
  ...
  const uploadMoreQuestions = async () => {
    ...
    try {
      ...
      if (response.data) { toast.success("Added more questions!"); fetchSessionDetailsById(); }
    } catch (error) {
      setErrorMsg(error.response?.data?.message || "Something went wrong. Please try again");
      toast.error("Failed to add more questions");
    } finally { setIsUpdateLoader(false); }
  };

  useEffect(() => { if (sessionId) fetchSessionDetailsById(); }, []);
            ...
            {sessionData?.questions?.length > 0 && (
              ...
              >
                <Suspense fallback={<div>Loading...</div>}>
                  {isUpdateLoader ? <SpinnerLoader size="sm" /> : <LuPlus className="text-lg" />}
                </Suspense>
                Load More
              </button>
            )}
          ...
          {!sessionData ? (
            <div className="space-y-4">
              {[1,2,3].map((item) => (
                <div key={item} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                  <Suspense fallback={<div>Loading...</div>}><SkeletonLoader /></Suspense>
                </div>
              ))}
            </div>
          ) : sessionData.questions?.length > 0 ? (
            <div className="space-y-5">
              <AnimatePresence mode="popLayout">
                {sessionData.questions.map((data, index) => (
                  <motion.div
                    ...
                    transition={{ duration: 0.3, type: "spring", stiffness: 100, delay: index*0.05, damping: 15 }}
              ...
              <div className="flex items-center justify-center pt-4">
                ...
                >
                  <Suspense fallback={<div>Loading...</div>}>
                    {isUpdateLoader ? <SpinnerLoader size="sm" /> : <LuListCollapse className="text-lg" />}
                  </Suspense>
                  Load More Questions
                </button>
              </div>
            ...
            >
              ...
              >
                <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between rounded-t-xl z-10">
                  ...
                  <button onClick={() => setShowExplanationModal(false)} className="text-gray-400 hover:text-gray-600 p-1 rounded-full hover:bg-gray-100">
                ...
                <div className="p-5 bg-white">
                  ...
                  <Suspense fallback={<SkeletonLoader count={3} />}>
                    {isLoading ? <SkeletonLoader count={3} /> : explanation ? <AIResponsePreview content={explanation?.explanation} /> :
